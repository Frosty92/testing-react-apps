...
Avoid implementation details because of False Negatives, and False positives.

What's an implementation detail ? It's when the changes to the code do not
affect the USER. Step back and understand why do we even write tests ? So the
app doesnt break ? What does that mean ? The end user is able to use the app.

The user doesnt care about the implementation, only the end result. Hence the
tests should do the same.

When testing implementation details, we can get false negatives (The test fails
but the app is working correctly), Or false positives (the tests pass, but the
app doesnt work)
